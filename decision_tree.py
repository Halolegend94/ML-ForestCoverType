    ...
    for m in min_saples_splits:
        for j in min_saples_leaf:
            for h in max_features:
                ...
                clf = DecisionTreeClassifier(criterion=c, min_samples_leaf=j, max_features=h, min_samples_split=m)
...
plt.title("Best decision tree configuration")
plt.xlabel("Configuration")
plt.ylabel("Score")
plt.savefig("Report/pictures/decision_tree_config.eps")
#start tests
clf = DecisionTreeClassifier(criterion=best_cri, max_features=best_f, min_samples_leaf=best_leaf, min_samples_split=best_spli)
startTest(main_dataset, dataset_parts, clf,"decision_tree")
